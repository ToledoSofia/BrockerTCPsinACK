...
public class Cliente1 {
    ...
    public void iniciarCliente() {
        try {
            ...
            //generar clave para criptografia simetrica
            KeyGenerator keySimetrica = KeyGenerator.getInstance("AES");
            keySimetrica.init(128); // for example
            SecretKey claveSimetrica = keySimetrica.generateKey();
            Hash.hashearSimetrica(claveSimetrica);

            /*ObjectOutputStream salida = new ObjectOutputStream(sc.getOutputStream());
            Mensaje mnsj = new Mensaje(Asimetrica.firmar(Hash.hashear(claveSimetrica).getBytes("UTF8"),privateKey,"RSA"),Asimetrica.encriptar(mensaje.getBytes("UTF8"),publicaDestino,"RSA"));
            salida.writeObject(publicKey);
            salida.flush();
            System.out.println("se mando clave simetrica");*/
